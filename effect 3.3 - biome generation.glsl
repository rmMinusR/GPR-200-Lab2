...
//This is meant to be the base of a world generator containing
//3 biomes. Each is denoted by red, green, or blue.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float f0 = fractalNoise(access_pos.xyz, 9, pow(1.61803399, -1.), 1.2, SEED);
    float f1 = fractalNoise(access_pos.xyz, 9, pow(1.61803399, -1.), 1.2, SEED<<2);
    float f2 = fractalNoise(access_pos.xyz, 9, pow(1.61803399, -1.), 1.2, SEED>>2);
    
    //Attempt to fix distribution
    //Not ideal but "good enough"
    f0 = curve(clamp01(fmap(f0, 0.25, 0.75, 0., 1.)), 3.2);
    f1 = curve(clamp01(fmap(f1, 0.25, 0.75, 0., 1.)), 3.2);
    f2 = curve(clamp01(fmap(f2, 0.25, 0.75, 0., 1.)), 3.2);
    
    //vec3 col = HSVtoRGB(f0, f1, f2);
    vec3 col = vec3(0,0,0);
    if(f0 >= f1 && f0 > f2) col.x = 1.;
    if(f1 >= f2 && f1 > f0) col.y = 1.;
    if(f2 >= f0 && f2 > f1) col.z = 1.;
    
    /*
    //Crappy HSV substitute
    vec3 col = vec3(
        fmap(cos(f*2.*PI + 0.0/3.0*PI) , -1.0, 1.0, 0.0, 1.0),
        fmap(cos(f*2.*PI + 2.0/3.0*PI) , -1.0, 1.0, 0.0, 1.0),
        fmap(cos(f*2.*PI + 4.0/3.0*PI) , -1.0, 1.0, 0.0, 1.0)
    );
	// */
