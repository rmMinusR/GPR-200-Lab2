...
//I realized in post that perlin noise is not well suited for this,
//instead I should be using Voronoi noise. Unfortunately I don't know
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 warp = fractalNoise3(access_pos.xyz, 8, pow(1.61803399, -1.), 1.2, SEED);
    
    //Attempt to fix distribution
    //Not ideal but "good enough"
    warp.x = curve(clamp01(fmap(warp.x, 0.25, 0.75, 0., 1.)), 3.2) *2.-1.;
    warp.y = curve(clamp01(fmap(warp.y, 0.25, 0.75, 0., 1.)), 3.2) *2.-1.;
    warp.z = curve(clamp01(fmap(warp.z, 0.25, 0.75, 0., 1.)), 3.2) *2.-1.;
    
    // Output to screen
    fragColor = texture(iChannel0, fragCoord/iResolution.xy + 0.025*warp.xy * warp.z);
}